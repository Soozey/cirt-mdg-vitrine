...
Le fichier `D:\cybersécurité\CLAUDE.md` a été vérifié à la demande du 2026-05-20. Il décrit l’ancienne plateforme complète de quiz/jobdating avec Firebase, Anthropic, admin, jury, authentification et banque de questions. Il avait été copié dans `public/documents/CLAUDE.md` et rendu consultable via `#/documents/claude`. Cette exposition publique a été retirée : le fichier est maintenant archivé dans `docs/archive/CLAUDE.md`.
...
- Correction de la date affichée : 23–24 Mai 2026.
- Correction de la date cible du compte à rebours : `2026-05-23T00:00:00+03:00`.
- Remplacement du compte à rebours initial par un bloc compact `Avant l’événement` avec cartes Jours, Heures, Minutes et Secondes.
- Ajout de pages de détail statiques pour les informations essentielles.
- Suppression de la route publique `#/documents/claude`, du composant d’affichage Markdown et du lien footer `Document MD`.
- Déplacement de `CLAUDE.md` hors du dossier public vers `docs/archive/CLAUDE.md`.
...
- Build final vérifié avec `npm.cmd run build`.
- Rendu desktop et mobile vérifié par capture Chrome headless.
- Vérification mobile sans débordement horizontal : `scrollWidth` égal à `clientWidth`.
